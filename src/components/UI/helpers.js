...
export const flutterwaveObj = (
  ...
  let obj = {
    ...
    subaccounts:
      settlement_type === "subaccount"
        ? [
            {
              id: sub_account_id,
              transaction_charge_type: "flat_subaccount",
              transaction_charge: totalAmount,
            },
          ]
        : [],
    meta: meta,
...
const maxAmount = (type, currentBalance, maxAmount) => {
  let amount = 0;
  if (type === "Percentage") {
    amount = (maxAmount / 100) * parseFloat(currentBalance);
  } else {
...
function generateReceiptNo(callback) {
  ...
  _fetchApi(
    `${apiURL}/account/getNextTransactionID`,
    ({ transactionId }) => {
      // let txnId = pad(transactionId, 6, 0)
      _fetchApi(
        `${apiURL}/account/getReceiptNo`,
        ({ receiptNo }) => {
...
const getCustomerInfo = (cb, customer_code, kyc_type) => {
  _fetchApi(
    `${apiURL}/kyc/query?query_type=select_kyc&query_param=${customer_code}&kyc_type=${kyc_type}`,
...
const getAgentDetails = (cd, obj, query_type) => {
  _postApi(
    `${apiURL}/claim/register-agent?query_type=${query_type}`,
    { obj },
    ({ results }) => {
      cd(results);
    },
